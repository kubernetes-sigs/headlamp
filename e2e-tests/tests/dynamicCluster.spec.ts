...
test(
  'excludes credentials from contexts other than the current one',
  { tag: KUBECONFIG_ONLY_TAG },
  async () => {
    const directory = await mkdtemp(join(tmpdir(), 'headlamp-kubeconfig-'));
    const kubeconfigPath = join(directory, 'config');
        ...
        clusters: [
          {
            ...
            cluster: {
              ...
              'certificate-authority-data': Buffer.from('current-ca').toString('base64'),
            },
          },
          {
            name: 'unrelated',
            cluster: {
              server: 'https://unrelated.example.com:6443',
              'certificate-authority-data': Buffer.from('unrelated-ca').toString('base64'),
            },
          },
        ],
        contexts: [
          { name: 'source', context: { cluster: 'source', user: 'source' } },
          { name: 'unrelated', context: { cluster: 'unrelated', user: 'unrelated' } },
        ],
        users: [
          {
            name: 'source',
            user: { 'client-key-data': Buffer.from('current-key').toString('base64') },
          },
          {
            name: 'unrelated',
            user: { token: 'unrelated-production-token' },
          },
        ],
        'current-context': 'source',
      };
      await writeFile(kubeconfigPath, yaml.stringify(multiContextKubeconfig));

      const encoded = await getBase64EncodedKubeconfig('scoped', kubeconfigPath);
      const decoded = Buffer.from(encoded, 'base64').toString();
      const normalized = yaml.parse(decoded);

      expect(normalized.clusters).toHaveLength(1);
      expect(normalized.users).toHaveLength(1);
      expect(normalized.contexts).toHaveLength(1);
      expect(normalized.clusters[0].cluster['certificate-authority-data']).toBe(
        Buffer.from('current-ca').toString('base64')
      );
      expect(decoded).not.toContain('unrelated-production-token');
      expect(decoded).not.toContain(Buffer.from('unrelated-ca').toString('base64'));
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  }
...
const getBase64EncodedKubeconfig = async (
  ...
  // Use kubectl command-line tool to get the kubeconfig. --minify keeps only
  // the current context so credentials for unrelated contexts are never
  // embedded or sent to the test Headlamp instance.
