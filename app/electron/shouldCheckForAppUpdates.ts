 ...
 *
 * @param environment - Environment variables used for development mode and as
 *   a configuration fallback.
 * @param resourcesPath - Directory containing packaged application resources.
 * @param developmentPath - Directory containing development application files.
 * @returns Whether the application should check for updates.
 */
export function shouldCheckForAppUpdates(
  environment: NodeJS.ProcessEnv = process.env,
  resourcesPath: string = process.resourcesPath,
  developmentPath: string = path.resolve('./')
): boolean {
  const manifestPath = path.join(
    environment.ELECTRON_DEV ? developmentPath : resourcesPath,
