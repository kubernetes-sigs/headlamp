...
/** A declared command scope paired with its opaque authorization token. */
interface PluginCommandCapability extends PluginCommandScope {
  /** Opaque authorization token generated by the main process. */
  capability: string;
}

/** Command scopes requested for one plugin. */
interface CommandCapabilityRegistration {
  /** Plugin package name. */
  pluginName: string;
  /** Command scopes declared by the plugin manifest. */
  scopes: PluginCommandScope[];
}

/** A validated command scope associated with its owning plugin. */
interface RegisteredCommandScope extends PluginCommandScope {
  /** Plugin package name that owns the scope. */
  pluginName: string;
}
 ...
 *
 * @param scope - Value supplied by the renderer.
...
/**
 * Validates plugin command registrations and creates opaque capabilities.
 *
 * @param registrations - Untrusted registrations supplied by the renderer.
 * @returns Public capabilities and the private scope lookup used for authorization.
 */
...
/**
 * Checks whether command data matches a registered capability scope.
 *
 * @param commandData - Validated command request.
 * @param scopeByCapability - Private capability-to-scope lookup.
 * @returns Whether the request is authorized and an error message when it is not.
 */
...
/**
 * Resolves an authorized scope for a command request.
 *
 * @param commandData - Validated command request.
 * @param scopeByCapability - Private capability-to-scope lookup.
 * @returns The matching scope, or undefined when the request is outside it.
 */
 ...
 * @param mainWindow - Window used as the consent dialog parent.
 * @param pluginName - Plugin identity used to isolate saved consent.
...
export function setupRunCmdHandlers(mainWindow: BrowserWindow | null, ipcMain: Electron.IpcMain) {
  ...
  ipcMain.on('run-command', (event, eventData) =>
    handleRunCommand(event, eventData, mainWindow, permissionSecrets, scopeByCapability)
  );
}
