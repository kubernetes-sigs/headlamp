...
APP_EXECUTABLE_NAME=$(node -e "const path=require('path'); const d=process.argv[1]; const p=require(path.join(d, 'package.json')); const {derivePackageVerificationName}=require(path.join(d, 'scripts/package-verification-name.cjs')); process.stdout.write(derivePackageVerificationName(p, 'linux'))" "$APP_DIR")
