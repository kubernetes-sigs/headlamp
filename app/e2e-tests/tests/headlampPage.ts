...
export class HeadlampPage {
  ...
  async a11y() {
    ...
    // belongs to an Electron BrowserWindow. This suite only ever drives
    // Electron (see playwright.electron.config.ts), so legacy mode is always
    // needed here; the coverage it gives up (cross-origin iframe scanning)
    // isn't a loss this suite would otherwise have.
  ...
  async startFromMainPage() {
    ...
    if (!currentURL.includes('c/minikube') && !currentURL.includes('c/main')) {
      ...
      await this.page.getByRole('link', { name: 'minikube', exact: true }).click();
      await this.page.waitForLoadState('load');
    }
  }
