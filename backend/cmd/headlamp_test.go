...
// setupFakeK8sServer create a mock k8s server for testing purpose,
// this would help to test Caching Machanism without making request
// to the k8s server.
func setupFakeK8sServer(authAllowed bool) *httptest.Server {
...
// setupHeadlampConfig create mock HeadlampConfig for testing CacheMiddleware
// mechanism without creating actual HeadlampConfig.
func setupHeadlampConfig(fakeK8s *httptest.Server, testName string) *HeadlampConfig {
...
func TestCacheMiddleware_CacheHitAndCacheMiss(t *testing.T) {
	...
	fakeK8s := setupFakeK8sServer(istrue)
	defer fakeK8s.Close()

	c := setupHeadlampConfig(fakeK8s, t.Name())
