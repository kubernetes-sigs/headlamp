...
func WriteToFile(config clientcmdapi.Config, path string) error {
	...
	if err != nil || !locked {
		if err == nil {
			err = errors.Errorf("failed to acquire file lock for %q within %s", configFile, timeoutForLock)
		}

		return errors.Wrapf(err, "locking kubeconfig file %q", configFile)
	}
...
func mergeWithExisting(config clientcmdapi.Config, dir, configFile string) (*clientcmdapi.Config, error) {
	...
	if n != len(data) {
		...
		return nil, errors.Errorf("short write to temp kubeconfig file: wrote %d of %d bytes", n, len(data))
	}

	if err = tmpFile.Close(); err != nil {
		return nil, errors.Wrap(err, "failed to close temp kubeconfig file")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to load merged kubeconfig")
	}
...
func RemoveContextFromFile(contextName string, path string) error {
	...
	if err != nil || !locked {
		if err == nil {
			err = errors.Errorf("failed to acquire file lock for %q within %s", path, timeoutForLock)
		}

		return errors.Wrapf(err, "locking kubeconfig file %q", path)
	}
