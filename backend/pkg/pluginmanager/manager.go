...
func (m *Manager) removeUndesired(desired map[string]DesiredPlugin) bool {
	...
	for name := range managed {
		...
		logger.Log(logger.LevelInfo, map[string]string{"plugin": name}, nil, "plugin manager: removing plugin")

		if err := remove(m.pluginsDir, name); err != nil {
			logger.Log(logger.LevelError, map[string]string{"plugin": name}, err, "plugin manager: removing plugin")
...
func (m *Manager) syncPlugin(ctx context.Context, plugin DesiredPlugin) bool {
	...
	if err := install(ctx, m.pluginsDir, plugin); err != nil {
		logger.Log(logger.LevelError, map[string]string{"plugin": plugin.Name}, err, "plugin manager: installing plugin")
