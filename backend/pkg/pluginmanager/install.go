...
// moves it into place under dir/plugin.Name.
func install(ctx context.Context, dir string, plugin DesiredPlugin) error {
	if err := os.MkdirAll(dir, dirPerm); err != nil {
		return err
	}

	archive, err := download(ctx, plugin)
	if err != nil {
		return err
	}
...
func download(ctx context.Context, plugin DesiredPlugin) (string, error) {
