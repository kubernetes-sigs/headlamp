...
func newCatalogClient(catalog Catalog) (catalogClient, error) {
...
func (c *indexCatalog) search(ctx context.Context, query string, limit, offset int) ([]CatalogEntry, error) {
	...
	matches := make([]CatalogEntry, 0, len(index.Plugins))

	for _, plugin := range index.Plugins {
		if !matchesQuery(query, plugin.Name, plugin.DisplayName, plugin.Description) {
			continue
		}
...
func (c *indexCatalog) resolve(ctx context.Context, entry CatalogEntry) (*ResolvedPlugin, error) {
	entries, err := c.search(ctx, "", maxSearchLimit, 0)
	if err != nil {
		return nil, err
	}
