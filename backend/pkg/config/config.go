...
const (
	...
	defaultSkipClusterBearerTokenPaths = "livez,readyz,healthz,version," +
...
type Config struct {
	...
	SkipClusterBearerTokenPaths  string `koanf:"skip-cluster-bearer-token-paths"`
...
func addProxyAuthFlags(f *flag.FlagSet) {
	...
	f.String("skip-cluster-bearer-token-paths", defaultSkipClusterBearerTokenPaths,
		"Comma-separated proxied cluster API paths that should be requested without authorization. "+
			"Headlamp removes any incoming Authorization header and does not inject a bearer token for these paths.")
}
