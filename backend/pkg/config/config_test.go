...
func TestParseBasic(t *testing.T) {
	tests := []struct {
		...
		{
			name: "with_args",
			args: []string{"go run ./cmd", "--port=3456", "-in-cluster", "--oidc-client-id=test", "--oidc-idp-issuer-url=https://example.com"},
			verify: func(t *testing.T, conf *config.Config) {
...
func TestParseWithEnv(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: []string{"go run ./cmd", "-in-cluster", "--port=9876"},
			env: map[string]string{
...
func TestParseFlags(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args: []string{"go run ./cmd", "--enable-dynamic-clusters", "-in-cluster"},
			verify: func(t *testing.T, conf *config.Config) {
