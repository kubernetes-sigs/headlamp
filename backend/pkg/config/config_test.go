...
func TestParseBasic(t *testing.T) { //nolint:funlen
	tests := []struct {
		name   string
		args   []string
	...
	}{
		{
			...
			verify: func(t *testing.T, conf *config.Config) {
				...
				assert.Equal(t,
					"livez,readyz,healthz,version,api/v1/namespaces/kube-public/configmaps/cluster-info",
					conf.SkipClusterBearerTokenPaths)
