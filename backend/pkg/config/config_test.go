...
func TestStaticOIDCEndpointsValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "all_required_static_endpoints_in_cluster",
			args:        []string{"go run ./cmd", "--in-cluster", authURL, tokURL, jwksURL},
			expectError: false,
		},
		{
			name:        "all_endpoints_including_optional_userinfo_in_cluster",
			args:        []string{"go run ./cmd", "--in-cluster", authURL, tokURL, jwksURL, infoURL},
			expectError: false,
		},
		{
			name:          "partial_required_endpoints_in_cluster",
			args:          []string{"go run ./cmd", "--in-cluster", authURL},
			expectError:   true,
			errorContains: "must all be set together",
		},
		{
			name:          "userinfo_without_required_endpoints_in_cluster",
			args:          []string{"go run ./cmd", "--in-cluster", infoURL},
			expectError:   true,
...
func TestStaticOIDCEndpointsValidationOutsideCluster(t *testing.T) {
	const (
		authURL = "--oidc-auth-url=https://idp.example.com/authorize"
		tokURL  = "--oidc-token-url=https://idp.example.com/token"
		jwksURL = "--oidc-jwks-url=https://idp.example.com/keys"
	...
	tests := []struct {
		...
		{
			name:          "complete_static_without_incluster",
			args:          []string{"go run ./cmd", authURL, tokURL, jwksURL},
			expectError:   true,
			errorContains: "only meant to be used in in-cluster mode",
		},
		{
			name:        "no_static_without_incluster",
			args:        []string{"go run ./cmd"},
			expectError: false,
		},
		{
			name:          "userinfo_only_without_incluster",
