...
func setEncodingHeaders(w http.ResponseWriter, encoding string) {
	...
	// Join all Vary values and search case-insensitively so we handle both
	// "Vary: Accept-Encoding" and "Vary: Origin, Accept-Encoding" correctly.
	if !strings.Contains(strings.ToLower(strings.Join(h["Vary"], ",")), "accept-encoding") {
...
func tryBrotliSidecar(w http.ResponseWriter, r *http.Request, absDir, sep string) bool {
	...
	if ctype == "" {
		// Extension not in MIME tables; sniff from the uncompressed file so
		// ServeFile doesn't guess from the .br bytes and emit a wrong type.
		if orig, err := os.ReadFile(absPath); err == nil { //nolint:gosec // path validated by filepath.Rel above
