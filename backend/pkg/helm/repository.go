...
func createFileIfNotThere(fileName string) error {
	...
	if os.IsNotExist(err) {
		...
		_ = os.Chmod(fileName, defaultNewConfigFileMode)
	}
...
func writeRepositoryFile(repoFile *repo.File, settings *cli.EnvSettings) error {
	// Tighten permissions if the file exists before writing
	err := os.Chmod(settings.RepositoryConfig, defaultNewConfigFileMode)
