...
you land": popup, popup-after-reload, and full-page redirect with no opener.
...
as three callers: anonymous, a namespace-scoped ServiceAccount, and the
cluster-admin ServiceAccount Headlamp runs as.

The comparison that matters is anonymous against the *namespace-scoped* account.
Against cluster-admin the two responses differ for uninteresting reasons; the
real question is whether an anonymous response can be told apart from a
legitimately unprivileged one by shape alone.
...
- The apiserver does not validate Dex tokens. These reproductions exercise
  ...
  RBAC — needs the flags in `kind-config.yaml` enabled and the cluster
  recreated after Dex is up.
