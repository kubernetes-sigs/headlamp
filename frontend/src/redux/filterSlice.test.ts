...
describe('filterSlice', () => {
  ...
  it('should restore namespaces and label selector when the cluster changes', () => {
    localStorage.setItem(
      'headlamp-selected-namespace_cluster-b',
      JSON.stringify(['payments', 'monitoring'])
    );
    localStorage.setItem('headlamp-label-selector_cluster-b', 'app=checkout,tier=frontend');
    state = {
      namespaces: new Set(['cluster-a-namespace']),
      labelSelector: 'cluster=a',
    };

    state = filterReducer(state, restoreFiltersForCluster('cluster-b'));

    expect(state.namespaces).toEqual(new Set(['payments', 'monitoring']));
    expect(state.labelSelector).toBe('app=checkout,tier=frontend');
  });
