            ...
            value: (
              <>
                ...
                {!!item?.jsonData?.spec?.limits?.[0]?.maxLimitRequestRatio &&
                  Object.keys(item.jsonData.spec.limits[0].maxLimitRequestRatio).length > 0 && (
                    <Box m={1}>
                      <Typography variant="h6">
                        {t('translation|Max Limit/Request Ratio')}
                      </Typography>
                      <MetadataDictGrid dict={item.jsonData.spec.limits[0].maxLimitRequestRatio} />
                    </Box>
                  )}
              </>
            ),
