...
export function EnvVarGrid(props: EnvVarGridProps) {
  ...
  const renderEnvVar = (envVar: any) => {
    if (envVar.valueFrom?.secretKeyRef) {
      ...
      const cluster = window.location.pathname.split('/')[2];
    ...
    if (envVar.valueFrom?.configKeyMapRef) {
      const { name: cmName, key: cmKey } = envVar.valueFrom.configKeyMapRef;

      return (
        <Typography>
          {envVar.name}: ConfigMap: {cmName} ({cmKey})
        </Typography>
      );
    }
