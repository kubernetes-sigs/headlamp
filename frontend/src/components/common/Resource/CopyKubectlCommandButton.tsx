...
/** Builds the `kubectl get <kind> <name> [-n <namespace>] -o yaml` command for a resource. */
export function getKubectlGetCommand(item: KubeObject): string {
  const parts = ['kubectl', 'get', item.kind.toLowerCase(), item.getName()];

  if (item.isNamespaced && item.getNamespace()) {
    parts.push('-n', item.getNamespace()!);
  }
