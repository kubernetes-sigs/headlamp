...
describe('MetadataDisplay', () => {
  ...
  it('copies multiple labels and annotations in the expected key=value format when the copy button is clicked', async () => {
    ...
    const copyLabelsBtn = screen.getByRole('button', { name: /Copy labels to clipboard/i });
    const copyAnnotationsBtn = screen.getByRole('button', {
      name: /Copy annotations to clipboard/i,
    });

    // Click the Labels copy button
    await userEvent.click(copyLabelsBtn);
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
      'app.kubernetes.io/name=my-app,environment=production'
    );

    // Click the Annotations copy button
    await userEvent.click(copyAnnotationsBtn);
