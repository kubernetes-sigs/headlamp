...
export interface ParsedLog {
  /** Timestamp parsed from kubernetes log line */
  timestamp: string;
  /** Detected severity of the line, info by default */
  severity: 'info' | 'error' | 'warning' | 'fatal' | 'trace' | 'debug';
  /** Log message */
  content: string;
  /**
   * Pod where this log came from
   * omitted if there's only one pod in the workload
   * */
  pod?: string;
}

// Build a lookup map for severity aliases
const severityLookupMap: Record<string, ParsedLog['severity']> = {};
for (const severityLevel in severityAliases) {
  const key = severityLevel as ParsedLog['severity'];
  for (const alias of severityAliases[key]) {
    severityLookupMap[alias] = key;
  }
}

const allAliases = Object.values(severityAliases).flat();
...
function parseLogLine(line: string, pod?: string): ParsedLog {
  ...
  let severity: ParsedLog['severity'] = 'info'; // Default severity
...
export const useParsedLogs = (logs: Record<string, string[]>) => {
