...
/** Mapping from ANSI escape codes to css class */
...
/** Parses text that contains ANSI codes into chunks of text with css classes */
...
export function AnsiText({
  ...
  const textElements = useMemo(() => {
    ...
    return parseAnsi(text).map((part, index) => {
      const content = highlightByRanges(
        part.text,
        currentOffset,
        highlightRanges,
        selectedHighlightRange
      );
      // eslint-disable-next-line react-hooks/immutability -- False positive facebook/react/issues/31569
  ...
  }, [text, highlightRanges, selectedHighlightRange]);
