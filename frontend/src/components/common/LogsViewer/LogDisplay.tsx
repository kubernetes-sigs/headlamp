...
/** Remove any ansi esacpe codes from the string */
const stripAnsi = (str: string) => str.replace(/\x1B[@-_][0-?]*[ -/]*[@-~]/g, '');
...
const useSearchLogs = (logs: ParsedLog[], query: string): SearchMatch[] => {
...
/** Component to display logs along with a search */
export function LogDisplay({
  ...
  logs: ParsedLog[];
  /** set of severities to display, shows all if not provided */
  severityFilter?: Set<string>;
  showTimestamps?: boolean;
  textWrap?: boolean;
  /** controls whether severity is displayed */
  ...
  useShortcut('LOG_VIEWER_SEARCH', () => setShowSearch(true));
  ...
  // Roughly estimate individual log height based on the content
  ...
  // When no search result is highlighted select first match from search results
  if (searchResults.length > 0 && !highlightedMatch) {
    setHighlightedMatch({ resultIndex: 0, matchIndex: 0 });
  }

  const highlightedResultIndex = useMemo(
    () => (highlightedMatch ? searchResults[highlightedMatch.resultIndex]?.index : undefined),
    [highlightedMatch, searchResults]
  );

  // Scroll to the highlighted item
  useEffect(() => {
    if (highlightedResultIndex === undefined || !listRef.current) return;

    listRef.current?.scrollToItem(highlightedResultIndex, 'start');
