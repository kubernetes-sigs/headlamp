...
import { useWorkloadLogs } from '../../../lib/k8s/api/v2/fetchLogs';
...
/** Display logs for a workload */
export function LogsViewer({ item, defaultSeverities, defaultContainer }: LogsViewerProps) {
  ...
  const [container, setContainer] = useState(() =>
    defaultContainer && containers.find(it => it.name === defaultContainer)
      ? defaultContainer
      : containers[0].name
  );
  const [lines, setLines] = useState<number | 'All'>(100);
  const { logs: allLogs, error: logsError } = useWorkloadLogs({
    item,
    container,
  ...
  const parsedLogs = useParsedLogs(allLogs);
  const logs = useMemo(
  ...
  function handleDownload() {
    ...
    try {
      ...
      console.error('Failed to download a file', e);
    } finally {
