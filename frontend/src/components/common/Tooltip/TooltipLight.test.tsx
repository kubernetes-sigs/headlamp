...
describe('TooltipLight', () => {
  ...
  it('renders newline-separated text preserving each line', async () => {
    ...
    // The tooltip surface itself is the element with the .MuiTooltip-tooltip class.
    // That is where our slotProps.tooltip.sx styles should be applied.
    const tooltipSurface = tooltip.className.includes('MuiTooltip-tooltip')
      ? tooltip
      : tooltip.querySelector('.MuiTooltip-tooltip');

    // Verify that the required styling actually reached the tooltip surface
    expect(tooltipSurface).toHaveStyle({ whiteSpace: 'pre-line' });
  });
