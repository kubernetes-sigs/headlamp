...
const { mockGetCombinedAllowedNamespaces, mockGetSavedNamespaces } = vi.hoisted(() => ({
  mockGetCombinedAllowedNamespaces: vi.fn(),
  mockGetSavedNamespaces: vi.fn(),
}));

vi.mock('../../helpers/clusterSettings', () => ({
  getCombinedAllowedNamespaces: (...args: any[]) => mockGetCombinedAllowedNamespaces(...args),
}));

vi.mock('../../lib/storage', () => ({
  getSavedNamespaces: (...args: any[]) => mockGetSavedNamespaces(...args),
  saveNamespaces: vi.fn(),
}));

describe('NamespacesAutocomplete', () => {
  beforeEach(() => {
    localStorage.clear();
    mockGetCombinedAllowedNamespaces.mockReturnValue([]);
    mockGetSavedNamespaces.mockReturnValue([]);
  ...
  it('renders PureNamespacesAutocomplete when allowedNamespaces are set in cluster settings', async () => {
    mockGetCombinedAllowedNamespaces.mockReturnValue(['allowed-ns-1', 'allowed-ns-2']);
    mockGetSavedNamespaces.mockReturnValue(['allowed-ns-1']);
  ...
  it('clears stale namespaces from filter state when loaded namespace list is empty', async () => {
    ...
    // Set initial filter with TWO stale namespaces (proves all are cleared, not just one)
    store.dispatch(setNamespaceFilter(['stale-namespace-1', 'stale-namespace-2']));
