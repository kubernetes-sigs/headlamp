...
describe('ReadyStatusLabel', () => {
  ...
  it('builds a tooltip title from label, reason, and message, on separate lines', async () => {
    renderWithContext(
  ...
  it('is keyboard-focusable and shows the tooltip on focus when there is extra info', async () => {
    renderWithContext(
      <ReadyStatusLabel status="False" reason="CrashLoopBackOff" message="container exited" />
    );
    const chip = screen.getByText('Not Ready');
    await userEvent.tab();
    expect(chip.closest('[tabindex]')).toHaveFocus();
    const tooltip = await screen.findByRole('tooltip');
    expect(tooltip).toHaveTextContent('Reason: CrashLoopBackOff');
  });

  it('is not in the tab order when there is no extra info', () => {
    renderWithContext(<ReadyStatusLabel status="True" />);
