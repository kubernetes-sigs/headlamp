...
/**
 * Scan a single container's environment for references to the target resource.
 *
 * Both the `env` (single key) and `envFrom` (whole object) forms are checked, and each
 * matching entry is reported separately, so a container pulling two keys from the same
 * ConfigMap yields two usages.
 *
 * @param container The container to scan.
 * @param target Whether to look for a ConfigMap or a Secret reference.
 * @param name Name of the ConfigMap or Secret being looked for.
 * @param isInitOrEphemeral Whether this is an init or ephemeral container. Recorded on
 *                          each usage so callers can tell them apart from regular ones.
 *
 * @returns One usage per matching reference site, of kind `env` or `envFrom`.
 *          Empty if the container does not reference the resource.
 */
...
/**
 * Scan a Pod spec's volumes for references to the target resource.
 *
 * Covers both direct volume references and projected volume sources. Note that the two
 * spell the Secret reference differently: a direct Secret volume names it in `secretName`,
 * while a projected Secret source uses `name` like every other reference does.
 *
 * @param spec The Pod spec whose `volumes` are scanned.
 * @param target Whether to look for a ConfigMap or a Secret reference.
 * @param name Name of the ConfigMap or Secret being looked for.
 *
 * @returns One usage per matching volume, of kind `volume` for a direct reference or
 *          `projected` for a projected source. Empty if no volume references the resource.
 */
