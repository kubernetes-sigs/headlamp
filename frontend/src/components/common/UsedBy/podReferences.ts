...
export interface PodUsage {
  ...
  container?: string;
  /** True if the container is an init or ephemeral container. */
...
function scanContainerEnv(
  ...
  for (const entry of container.env ?? []) {
    ...
    if (ref?.name === name) {
      usages.push({ kind: 'env', container: container.name, initContainer });
    }
  }
  ...
  for (const entry of container.envFrom ?? []) {
    ...
    if (ref?.name === name) {
      usages.push({ kind: 'envFrom', container: container.name, initContainer });
    }
  }
