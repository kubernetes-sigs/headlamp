...
function containerLabel(usage: PodUsage, t: (key: string) => string): string {
  return usage.isInitOrEphemeral
    ? `${usage.container} (${t('translation|init/ephemeral')})`
    : `${usage.container}`;
}
...
function usageLabel(usage: PodUsage, t: (key: string) => string): string {
  switch (usage.kind) {
    case 'env':
      return `${t('translation|env')} (${containerLabel(usage, t)})`;
    case 'envFrom':
      return `${t('translation|envFrom')} (${containerLabel(usage, t)})`;
    case 'volume':
  ...
  return (
    <Stack direction="row" flexWrap="wrap" gap={0.5}>
      {usages.map((usage, index) => (
        <Chip
          key={`${usage.kind}-${usage.container ?? ''}-${usage.volume ?? ''}-${
            usage.isInitOrEphemeral ? 'init' : 'main'
          }-${index}`}
          label={usageLabel(usage, t)}
          size="small"
          variant="outlined"
          color={usage.isInitOrEphemeral ? 'default' : 'primary'}
        />
      ))}
    </Stack>
  );
    ...
    <SimpleTable
      ...
      columns={[
        ...
        },
        {
          ...
          getter: (row: PodReference) => makePodStatusLabel(row.pod, false, t),
        },
        {
          label: t('glossary|Owner'),
...
export default function UsedBySection({ resource, resourceKind }: UsedBySectionProps) {
  ...
  const {
    ...
    // Poll instead of opening a long-lived watch for all Pods in the namespace.
