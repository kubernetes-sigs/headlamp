...
describe('findPodReferences', () => {
  ...
  describe('ConfigMap reference paths', () => {
    it('finds env.valueFrom.configMapKeyRef', () => {
      ...
      expect(refs[0].usages).toEqual([{ kind: 'env', container: 'app', initContainer: false }]);
    });
    ...
    it('finds envFrom.configMapRef', () => {
      ...
      const refs = findPodReferences([pod], 'ConfigMap', 'my-cm');
      expect(refs[0].usages).toEqual([{ kind: 'envFrom', container: 'app', initContainer: false }]);
    });

    it('finds initContainers env/envFrom and marks initContainer=true', () => {
      const pod = makePod({
        spec: {
      ...
      const refs = findPodReferences([pod], 'ConfigMap', 'my-cm');
      expect(refs[0].usages).toEqual([{ kind: 'envFrom', container: 'init', initContainer: true }]);
    });

    it('finds ephemeralContainers env/envFrom and marks initContainer=true', () => {
      const pod = makePod({
        spec: {
          containers: [{ name: 'app', image: 'nginx' }],
          ephemeralContainers: [
      ...
      expect(refs[0].usages).toEqual([
        { kind: 'envFrom', container: 'debug', initContainer: true },
      ]);
    });
  ...
  describe('Secret reference paths', () => {
    it('finds env.valueFrom.secretKeyRef', () => {
      ...
      expect(findPodReferences([pod], 'Secret', 'my-secret')[0].usages).toEqual([
        { kind: 'env', container: 'app', initContainer: false },
      ]);
    });
    ...
    it('finds envFrom.secretRef', () => {
      ...
      expect(findPodReferences([pod], 'Secret', 'my-secret')[0].usages).toEqual([
        { kind: 'envFrom', container: 'app', initContainer: false },
      ]);
    });
