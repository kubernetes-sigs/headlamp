...
describe('UsedBySection', () => {
  ...
  it('names the container category in the chip text, not only in its colour', () => {
    const pod = makePod({
      name: 'consumer',
      ownerKind: 'Deployment',
      ownerName: 'web',
      spec: {
        ...
        initContainers: [
          { name: 'setup', image: 'busybox', envFrom: [{ configMapRef: { name: 'my-cm' } }] },
        ],
      },
    });
    podUseListMock.mockReturnValue({ items: [pod], errors: null, isLoading: false });
    ...
    expect(screen.getByText('envFrom (setup (init/ephemeral))')).toBeInTheDocument();
    expect(screen.getByText('envFrom (app)')).toBeInTheDocument();
  });

  it('shows no owner when the Pod has only non-controller owner references', () => {
    const pod = makePod({
      name: 'consumer',
      spec: {
        containers: [
    ...
    expect(screen.getByText('consumer')).toBeInTheDocument();
    expect(screen.queryByText('Deployment/web')).not.toBeInTheDocument();
  });
});
