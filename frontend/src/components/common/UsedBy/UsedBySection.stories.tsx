...
// Stories below render against an msw-backed Pod list. The storyshots runner
// waits for all msw requests and React Query fetches to settle before
// snapshotting, so it captures the populated table / error these stories
// demonstrate. We still opt out of storyshots here: the populated and error
// UIs are already covered by UsedBySection.test.tsx, so snapshotting them only
// adds large, redundant snapshots to maintain. The Loading story below must
// also stay opted out because it uses an infinite delay that would otherwise
// hang the snapshot runner.
