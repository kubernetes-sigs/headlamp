...
describe('ValidatingAdmissionPolicyBindingDetails', () => {
  ...
  it('renders extraSections with matchResources', () => {
    render(
      <TestContext routerMap={{ name: 'demo-binding' }}>
        <ValidatingAdmissionPolicyBindingDetails />
      </TestContext>
    ...
    const props = mockDetailsGrid.mock.calls[0][0];
    const extraSections = props.extraSections(fakeBinding);

    const sectionIds = extraSections.map((s: any) => s.id);
    expect(sectionIds).toContain('headlamp.validatingadmissionpolicybinding.matchResources');
  });

  it('renders paramRef selector when present', () => {
    const bindingWithSelector = {
      ...fakeBinding,
      spec: {
        ...fakeBinding.spec,
        paramRef: {
    ...
    const byName = Object.fromEntries(
      props
        .extraInfo(bindingWithSelector)
        .map((f: any) => [String(f.name).split('|').pop(), f.value])
    );

    expect(byName['Param Ref Selector']).toBeDefined();
  });
});
