...
// Mock TileChart to render slices as accessible buttons for interaction testing
  ...
  default: ({
    ...
    <div>
      ...
      {(data || []).map(slice => (
        <button
          key={slice.name}
          aria-label={slice.label ?? slice.name}
          tabIndex={0}
          onClick={slice.onClick}
          onKeyDown={e => {
            if ((e.key === 'Enter' || e.key === ' ') && slice.onClick) slice.onClick();
          }}
        >
...
describe('StatusCharts', () => {
  it('renders PodsStatusCircleChart and navigates on mouse click and keyboard input (Enter/Space)', () => {
    ...
    const readySlice = screen.getByRole('button', { name: 'Ready' });
    expect(readySlice).toHaveAttribute('tabIndex', '0');

    // Test mouse click
    fireEvent.click(readySlice);
    expect(currentUrl).toBe('/c/story-cluster/pods?podsfilter=Running');

    // Test keyboard Enter key
    fireEvent.keyDown(readySlice, { key: 'Enter' });
    expect(currentUrl).toBe('/c/story-cluster/pods?podsfilter=Running');

    const notReadySlice = screen.getByRole('button', { name: 'Not Ready' });
  ...
  it('renders NodesStatusCircleChart and navigates on mouse click and keyboard input (Enter/Space)', () => {
    ...
    // Test mouse click
    fireEvent.click(readySlice);
    expect(currentUrl).toBe('/c/story-cluster/nodes?nodesfilter=Yes');
    ...
    // Test mouse click
    fireEvent.click(notReadySlice);
    expect(currentUrl).toBe('/c/story-cluster/nodes?nodesfilter=No');
