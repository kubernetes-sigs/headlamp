...
export const NodeGlance = ({ node }: { node: GraphNode }) => {
  ...
  const glanceResults = Object.values(glances).map(glance => {
    ...
    return (
      <Fragment key={glance.id}>
        <GlanceComponent node={node} />
      </Fragment>
    );
  });

  const validGlanceResults = glanceResults.filter(result => result !== null);
  if (validGlanceResults.length > 0) {
    return <>{validGlanceResults}</>;
  }

  // Render KubeObjectGlance for Kubernetes objects
  if (node.kubeObject instanceof KubeObject) {
    return <KubeObjectGlance resource={node.kubeObject} />;
  }

  // Fallback for nodes without a specific glance
  return null;
};
