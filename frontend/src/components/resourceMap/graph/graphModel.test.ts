...
describe('getNodeWeight', () => {
  it('correctly assigns default weights to different Kubernetes resource types', () => {
    const testCases = [
      ...
      { kind: 'StatefulSet', expectedWeight: 970 },
      { kind: 'DaemonSet', expectedWeight: 960 },
      { kind: 'CronJob', expectedWeight: 940 },
      { kind: 'Job', expectedWeight: 920 },
      { kind: 'ReplicaSet', expectedWeight: 850 },
      { kind: 'Pod', expectedWeight: 800 },
      { kind: 'Ingress', expectedWeight: 780 },
      { kind: 'Service', expectedWeight: 760 },
      { kind: 'ConfigMap', expectedWeight: 580 },
      { kind: 'Secret', expectedWeight: 570 },
      { kind: 'ServiceAccount', expectedWeight: 550 },
      { kind: 'UnknownResource', expectedWeight: 300 }, // default weight
    ];
  ...
  it('uses default weight for nodes without kubeObject', () => {
    ...
    expect(getNodeWeight(node)).toBe(300);
  });
});
