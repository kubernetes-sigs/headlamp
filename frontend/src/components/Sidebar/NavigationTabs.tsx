...
import { fullURLOnRoute } from './SidebarItem';
...
function findIndexNameInSubList(sublist: SidebarItemProps['subList'], name: string): number {
  if (!sublist) {
    return -1;
  }
  for (let i = 0; i < sublist.length; i++) {
...
function findIndexParentOfSubList(list: SidebarItemProps[], name: string | null): number | null {
  if (!name) {
    return null;
  }
...
export default function NavigationTabs() {
  ...
  useEffect(() => {
    ...
    const index = findIndexParentOfSubList(subList, sidebar.selected.item) ?? null;
  ...
  const tabChangeHandler = useCallback(
    (index: number) => {
      ...
      if (item.url && item.useClusterURL && getCluster()) {
        ...
        history.push({ pathname: fullURLOnRoute(item.name, item.isCR, item.subList ?? []) });
      }
    },
