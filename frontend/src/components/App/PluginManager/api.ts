...
async function managerFetch(path: string): Promise<Response> {
  const response = await fetch(`${getAppUrl()}${path}`, {
    headers: new Headers(addBackstageAuthHeaders()),
  });
  if (!response.ok) {
...
export function usePluginManagerInfo(refetchIntervalMs: number = 5000) {
...
export async function searchCatalog(
  ...
  const result = (await response.json()) as { entries: CatalogEntry[] };
