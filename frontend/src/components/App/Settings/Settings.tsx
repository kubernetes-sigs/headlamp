...
export default function Settings() {
  ...
  const expandGraphSetting = useSetting<boolean>('expandLargeGraph');
  ...
  const [expandGraph, setExpandGraph] = useState<boolean>(expandGraphSetting.value ?? false);
  ...
  useEffect(() => {
    setExpandGraph(expandGraphSetting.value ?? false);
  }, [expandGraphSetting.value]);
  ...
  useEffect(() => {
    if (!expandGraphSetting.disabled) {
      dispatch(
        setAppSettings({
      ...
      <NameValueTable
        rows={[
            ...
            : []),
          {
            ...
            value: (
              <AdminControlled disabled={expandGraphSetting.disabled} t={t}>
                <Switch
                  color="primary"
                  checked={expandGraph}
                  onChange={e => setExpandGraph(e.target.checked)}
                  disabled={expandGraphSetting.disabled}
                  inputProps={{
            ...
            hide: expandGraphSetting.hidden,
          },
        ]}
      />
      {!themeDisplay.hidden && (
