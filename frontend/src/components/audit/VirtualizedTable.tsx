...
export default function VirtualizedTable({ events, regexFilter }: VirtualizedTableProps) {
  ...
  const Row = ({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    return (
      <TableRow component="div" style={style}>
        <TableCell component="div" sx={{ width: '20%' }}>
          {event.timestamp}
        </TableCell>
        <TableCell component="div" sx={{ width: '20%' }}>
          {event.user}
        </TableCell>
        <TableCell component="div" sx={{ width: '10%' }}>
          {event.verb}
        </TableCell>
        <TableCell component="div" sx={{ width: '15%' }}>
          {event.resource}
        </TableCell>
        <TableCell component="div" sx={{ width: '10%' }}>
          {event.statusCode}
        </TableCell>
        <TableCell component="div" sx={{ width: '25%' }}>
          {event.message}
        </TableCell>
      </TableRow>
    );
  ...
  return (
    <Box sx={{ height: '100%', width: '100%' }}>
      ...
      {filteredEvents.length === 0 && <Typography sx={{ p: 2 }}>No audit events found.</Typography>}
    </Box>
  );
}
