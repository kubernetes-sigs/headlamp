...
export function SingleActivityRenderer({
  ...
  // Handle click outside to close or minimize temporary activities, respecting pin state
  ...
  useEffect(() => {
    if (!activity.temporary || minimized || isOverview || isDetailsDrawerOpen) {
      return;
    }
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!activityElement.contains(event.target as Node)) {
        ...
        // 2. Resource links (let Link.tsx handle the transition) -- but not links that are
        // part of the global navigation chrome (sidebar, top app bar), since virtually every
        // in-app link matches `a[href*="/"]` under HashRouter and would otherwise make the
        // sidebar/logo effectively immune to click-outside.
        const isResourceLink =
          !!target.closest('a[href*="/"], a[role="button"]') && !target.closest('nav, header');
...
export const ActivitiesRenderer = React.memo(function ActivitiesRenderer() {
  ...
  // Close or minimize the last activity when ESC is pressed, but only for temporary
  // resource-detail activities -- not terminals, log viewers, editors, or other
  // activities the user launched explicitly and expects ESC to behave normally in.
  useHotkeys('Escape', () => {
    const lastActivity = lastElement
      ? activities.find(activity => activity.id === lastElement)
      : undefined;
    if (lastActivity?.temporary && !isOverview) {
      Activity.closeOrMinimize(lastActivity.id);
    }
  });
          ...
          <Tooltip title={it.pinned ? t('Unpin') : t('Pin')}>
            <IconButton
              ...
              onClick={e => {
                e.preventDefault();
                e.stopPropagation();
                // Preserve the current minimized state explicitly -- Activity.update
                // treats an omitted `minimized` as false, which would wrongly
                // un-minimize (and bump into history) an already-minimized activity.
                Activity.update(it.id, { pinned: !it.pinned, minimized: it.minimized });
              }}
              sx={{
                width: '42px',
                height: '100%',
