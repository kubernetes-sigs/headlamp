    ...
    <DetailsGrid
      ...
      extraInfo={item =>
        item && [
          ...
          {
            name: t('Host IPs'),
            value: item.status.hostIPs ? item.status.hostIPs.map(ipObj => ipObj.ip).join(', ') : '',
          ...
          {
            name: t('Pod IPs'),
            value: item.status.podIPs ? item.status.podIPs.map(ipObj => ipObj.ip).join(', ') : '',
            hideLabel: item.status.podIPs?.length === 0,
          },
          {
            name: t('QoS Class'),
