...
function replaceUseId(node: any) {
  ...
  // React wraps useId values in colons, e.g. ":r3j2:". The attributes above only ever
  // hold generated ids, so a bare colon is enough to spot one. A name can legitimately
  // contain a colon ("settings:mode"), so it is matched on the full shape instead.
  const reactGeneratedId = /^:[^:]*:$/;
  if (node.nodeType === Node.ELEMENT_NODE) {
    for (const attr of node.attributes) {
      if (attr.name === 'name') {
        if (reactGeneratedId.test(attr.value)) {
          // MuiRadioGroup and friends fall back to useId when no name prop is given
          node.setAttribute(attr.name, ':mock-test-id:');
        }
      } else if (attributesToReplace.includes(attr.name)) {
...
describe('replaceUseId', () => {
  function inputWith(attributes: Record<string, string>) {
    // Detached on purpose: the storyshots below snapshot document.body.
  ...
  test('replaces a name generated by useId', () => {
    const element = inputWith({ name: ':r3j2:' });
    replaceUseId(element);
    expect(element.getAttribute('name')).toBe(':mock-test-id:');
  });

  test('keeps a name that merely contains a colon', () => {
    const element = inputWith({ name: 'settings:mode' });
    replaceUseId(element);
    expect(element.getAttribute('name')).toBe('settings:mode');
  });

  test('keeps an ordinary name', () => {
    const element = inputWith({ name: 'storage-class-mode' });
    replaceUseId(element);
    expect(element.getAttribute('name')).toBe('storage-class-mode');
  });

  test('still replaces an id generated by useId', () => {
    const element = inputWith({ id: ':r1:' });
    replaceUseId(element);
    expect(element.getAttribute('id')).toBe(':mock-test-id:');
  });
});
