 ...
 *  4. Default: `'websockets'` once runtime config has loaded. When no user or environment override
 *     is set, returns `'off'` while runtime config is still `null` (before the `/config` response
 *     arrives) to avoid opening WebSocket connections before the server-configured mode is known.
 *
...
export function useWatchKubeObjectLists<K extends KubeObject>({
  ...
  useWatchKubeObjectListsMultiplexed({
    ...
    queryParams: mode === 'multiplexer' ? queryParams : undefined,
    watchQueryParams: mode === 'multiplexer' ? watchQueryParams : undefined,
  ...
  useWatchKubeObjectListsLegacy({
    ...
    queryParams: mode === 'websockets' ? queryParams : undefined,
    watchQueryParams: mode === 'websockets' ? watchQueryParams : undefined,
