...
import App from '../../../../App';
import { makeBatchingStream, makeLineSplitStream } from './readableLogStream';
...
describe('makeLineSplitStream', () => {
  ...
  it('handles mixed LF and CRLF line endings', async () => {
    const stream = new ReadableStream<string>({
      start(controller) {
        controller.enqueue('lf-line\ncrlf-line\r\nlast');
        controller.close();
      },
    });

    const result = await collectStream(stream.pipeThrough(makeLineSplitStream()));
    expect(result).toEqual(['lf-line', 'crlf-line', 'last']);
  });

  it('preserves CR characters in the middle of a line', async () => {
    const stream = new ReadableStream<string>({
      start(controller) {
        controller.enqueue('progress: 10%\rprogress: 20%\r\n');
        controller.close();
      },
    });

    const result = await collectStream(stream.pipeThrough(makeLineSplitStream()));
    expect(result).toEqual(['progress: 10%\rprogress: 20%']);
  });

  it('strips trailing CR from leftover flushed on close', async () => {
    const stream = new ReadableStream<string>({
      start(controller) {
        controller.enqueue('line1\r\npartial\r');
        controller.close();
