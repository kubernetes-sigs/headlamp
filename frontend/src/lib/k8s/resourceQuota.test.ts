...
describe('ResourceQuota', () => {
  describe('counts', () => {
    ...
    it('keeps count/ keys as counts whatever the resource is named', () => {
      const quota = makeQuota({
        'count/requests.example.com': '3',
        'count/limits.example.com': '2',
      });

      expect(quota.requests).toEqual([]);
      expect(quota.limits).toEqual([]);
      expect(quota.counts).toEqual([
        'count/requests.example.com: 0/3',
        'count/limits.example.com: 0/2',
      ]);
    });
