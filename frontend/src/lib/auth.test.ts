...
describe('auth', () => {
  ...
  describe('setToken', () => {
    it('should successfully set a token for a cluster', async () => {
      ...
      expect(mockBackendFetch).toHaveBeenCalledWith('/auth/set-token', {
        method: 'POST',
        headers: {
    ...
    it('should successfully clear a token when token is null', async () => {
      ...
      expect(mockBackendFetch).toHaveBeenCalledWith('/auth/set-token', {
        ...
        body: JSON.stringify({ cluster: cluster.trim(), token: null }),
      });
    });
  ...
  describe('logout', () => {
    it('should successfully logout by clearing the token', async () => {
      // Arrange
      const cluster = 'test-cluster';
      const mockResponse = {
        ok: true,
