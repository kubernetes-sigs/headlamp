...
/**
 * A React component which configures the monaco-editor component to load using local sources
 */
export function MonacoEditorProvider({ children }: React.PropsWithChildren<{}>) {
  ...
  useState(() => {
    ...
    const url = `${window.location.origin}/${getBaseUrl()}assets/vs`;
    loader.config({
      paths: {
        vs: url,
      },
