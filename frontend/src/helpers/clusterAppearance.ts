 ...
 * @deprecated Appearance is now stored in localStorage via clusterSettings
 */
 ...
 *
 * @param clusterNameOrMetaData - The cluster name, or for backward compatibility, cluster metadata.
...
export function getClusterAppearanceFromMeta(
  clusterNameOrMetaData: string | ClusterMetadata | undefined
): ClusterAppearance {
  // Handle both old signature (metaData) and new signature (clusterName)
  let clusterName: string | undefined;

  if (typeof clusterNameOrMetaData === 'string') {
    clusterName = clusterNameOrMetaData;
  } else {
    // Old signature: first param is metadata, try to extract from window location
    clusterName = window.location.pathname.split('/c/')[1]?.split('/')[0];
  }
